...
def dose_at_response(y, einf, ec50, hs, e0):
...
class dose_response_curve:
    def __init__(self, compound, specimen, study_id, exp_id, model_type, param_bound, control_exp_flag, feature,
                 measure_before_norm, num_sec):
        ...
        self.param_bound = param_bound  # Bounds of parameters used in model fitting
    ...
    def curve_fit(self, dose, measure):
        """
        :param dose: should be in log10(M)
        :param measure:
        :return:
        """
        self.param, self.param_cov = None, None
        self.dose = dose
        self.measure = measure
        if self.dose is not None and self.measure is not None and len(self.dose) == len(self.measure):
            self.nfev = 100 * len(dose)
            while self.param is None and self.nfev < 100000:
                # print(self.nfev)
                try:
                    self.param, self.param_cov = curve_fit(response_curve_4p, self.dose, self.measure,
                                                         bounds=self.param_bound, max_nfev=self.nfev)
                except RuntimeError:
                    pass
                self.nfev *= 2
            # print(popt)
    ...
    def compute_fit_metrics(self, d1=-10, d2=-4, mode='trapz'):
