...
ht29_data = pd.read_csv('../../UCLA/Test_Study/HT29_Data.txt', low_memory=False, sep='\t', engine='c',
                        na_values=['na', '-', ''], header=0, index_col=None)
ht29_data.index = ht29_data.loc[:, 'A']
ht29_data = ht29_data.loc[:, ['_UCLA', '_Crown']]
ht29_data.columns = ['UCLA', 'Crown']

id = np.setdiff1d(list(range(ht29_data.shape[0])), np.intersect1d(np.where(ht29_data.loc[:, 'UCLA'] == '_')[0],
                                                                  np.where(ht29_data.loc[:, 'Crown'] == '_')[0]))
ht29_data = ht29_data.iloc[id, :]
compound = [i.split('_')[0] for i in ht29_data.index]
dose = [np.log10(float(i.split('_')[1])) - 6 for i in ht29_data.index]  # Dose should be log10 molar
col = []
...
ht29_data = pd.DataFrame({'Compound': compound, 'Dose': dose, 'Row': row, 'Column': col, 'Edge_Well': edge,
                          'UCLA': ht29_data.UCLA.astype(float), 'Crown': ht29_data.Crown.astype(float)})
id = np.where(ht29_data.Compound == 'PD0325901')[0]
ht29_data.iloc[id, 0] = 'PD-0325901'

dmso_id = np.where(ht29_data.Compound == 'DMSO')[0]
sts_id = np.where(ht29_data.Compound == 'Staurosporine')[0]

data = {}
data['HT29'] = {}

# # Detect IQR outliers and edge outliers in UCLA protocol data
# dmso_outlier_id, lower, upper = IQR_outlier_detection(ht29_data.iloc[dmso_id, :].UCLA.values)
# sts_outlier_id, lower, upper = IQR_outlier_detection(ht29_data.iloc[sts_id, :].UCLA.values)
# edge_outlier_id = edge_outlier_detection(ht29_data, 'UCLA')
# outlier_id = np.union1d(np.union1d(dmso_id[dmso_outlier_id], sts_id[sts_outlier_id]), edge_outlier_id)
# ht29_data['Outlier_UCLA'] = [True if i in outlier_id else False for i in range(ht29_data.shape[0])]
# id_norm = np.where(np.invert(ht29_data.Outlier_UCLA))[0]
# data['HT29']['UCLA'] = ht29_data.iloc[id_norm, :].loc[:, ['Compound', 'Dose', 'Row', 'Column', 'UCLA']]

data['HT29']['UCLA'] = ht29_data.loc[:, ['Compound', 'Dose', 'Row', 'Column', 'UCLA']]
data['HT29']['UCLA'].columns = ['Compound', 'Dose', 'Row', 'Column', 'Data']

# # Detect IQR outliers and edge outliers in Crown protocol data
# dmso_outlier_id, lower, upper = IQR_outlier_detection(ht29_data.iloc[dmso_id, :].Crown.values)
# sts_outlier_id, lower, upper = IQR_outlier_detection(ht29_data.iloc[sts_id, :].Crown.values)
# edge_outlier_id = edge_outlier_detection(ht29_data, 'Crown')
# outlier_id = np.union1d(np.union1d(dmso_id[dmso_outlier_id], sts_id[sts_outlier_id]), edge_outlier_id)
# ht29_data['Outlier_Crown'] = [True if i in outlier_id else False for i in range(ht29_data.shape[0])]
# id_norm = np.where(np.invert(ht29_data.Outlier_Crown))[0]
# data['HT29']['Crown'] = ht29_data.iloc[id_norm, :].loc[:, ['Compound', 'Dose', 'Row', 'Column', 'Crown']]

data['HT29']['Crown'] = ht29_data.loc[:, ['Compound', 'Dose', 'Row', 'Column', 'Crown']]
data['HT29']['Crown'].columns = ['Compound', 'Dose', 'Row', 'Column', 'Data']
    ...
    for plate in data[cell].keys():
        ...
        for compound in pd.unique(data[cell][plate].Compound):
            ...
            fit = dose_response_curve(compound=compound, specimen=cell, study_id='UCLA_Test',
                                      exp_id=plate + '_' + str(exp), model_type='4_parameter_hillslop',
...
fitted_data_key = list(fitted_data)
...
for i in range(len(cell_l)):
    ...
    for k in ori_data_key:
        if label in k and 'Fraction_living_cells' in k:
